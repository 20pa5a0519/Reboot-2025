...
username = "neo4j"
password = "Genaifraud" # Replace with your actual Neo4j password
        ...
        with driver.session() as session:
            ...
            WITH a, t, b, collusion_b,
                 collect(DISTINCT {
                     ...
                     properties: {
                         ...
                         Predicted_fraud_flag_t1: t1.Predicted_Fraud_Flag, // Add fraud details from contributing transactions
                         Predicted_risk_score_t1: t1.Predicted_Risk_Score,
                         Predicted_fraud_flag_t2: t2.Predicted_Fraud_Flag,
                         Predicted_risk_score_t2: t2.Predicted_Risk_Score
                     }
            ...
            RETURN
                ...
                CASE
                    ...
                    WHEN a.is_Predicted_fraud = true THEN 'red'
                    ELSE 'green'
                END AS a_color,
                t,
                // Calculate t_color based on Fraud_Flag and Risk_Score on the transaction relationship
                CASE
                    WHEN t.Predicted_Fraud_Flag = 1 OR t.Predicted_Risk_Score > 3 THEN 'red'
                    ELSE 'green'
                END AS t_color,
                b,
                CASE 
                    WHEN b.Predicted_Fraud_Flag = 1 OR b.Predicted_Risk_Score > 3 THEN 'red'
                    ELSE 'green'
                END AS b_color,
            ...
            for record in result:
                ...
                def add_node_if_new(node_obj, color_override=None):
                    ...
                    if node_id_val and node_id_val not in node_ids:
                        ...
                        if node_data.get('is_Predicted_fraud') is not None:
                            tooltip_html += f"<br><strong>Predicted_Fraudulent:</strong> {'Yes' if node_data['is_Predicted_fraud'] else 'No'}"
                            if node_data['is_Predicted_fraud']:
                                if node_data.get('Predicted_Fraud_Flag') is not None: tooltip_html += f"<br><strong>Predicted_Fraud Flag:</strong> {node_data['Predicted_Fraud_Flag']}"
                                if node_data.get('Predicted_Risk_Score') is not None: tooltip_html += f"<br><strong>Predicted_Risk Score:</strong> {node_data['Predicted_Risk_Score']}"
                                if node_data.get('Predicted_fraud_reason'): tooltip_html += f"<br><strong>Reason:</strong> {node_data['Predicted_fraud_reason']}"
                                if node_data.get('Predicted_fraud_type'): tooltip_html += f"<br><strong>Type:</strong> {node_data['Predicted_fraud_type']}"
                        ...
                        for key, value in node_data.items():
                            if key not in ['id', 'label', 'color', 'account_holder', 'phone', 'is_Predicted_fraud',
                                           'ip_address', 'city', 'title', 'Predicted_Fraud_Flag', 'Risk_Score',
                                           'Predicted_fraud_reason', 'Predicted_fraud_type']: # Exclude 'title' itself
                ...
                def add_edge_if_new(from_obj, to_obj, label, color, properties=None):
                    ...
                    if edge_key not in edge_keys:
                        ...
                        if label == 'TRANSACTION':
                            if edge_data.get('Predicted_Fraud_Flag') is not None: tooltip_html += f"<br><strong>Predicted_Fraud Flag:</strong> {edge_data['Predicted_Fraud_Flag']}"
                            if edge_data.get('Predicted_Risk_Score') is not None: tooltip_html += f"<br><strong>Predicted_Risk Score:</strong> {edge_data['Predicted_Risk_Score']}"
                            ...
                            if edge_data.get('Predicted_fraud_flag_t1') is not None: tooltip_html += f"<br><strong>Trans 1 Predicted_Fraud Flag:</strong> {edge_data['Predicted_fraud_flag_t1']}"
                            if edge_data.get('Predicted_risk_score_t1') is not None: tooltip_html += f"<br><strong>Trans 1 Predicted_Risk Score:</strong> {edge_data['Predicted_risk_score_t1']}"
                            if edge_data.get('Predicted_fraud_flag_t2') is not None: tooltip_html += f"<br><strong>Trans 2 Predicted_Fraud Flag:</strong> {edge_data['Predicted_fraud_flag_t2']}"
                            if edge_data.get('Predicted_risk_score_t2') is not None: tooltip_html += f"<br><strong>Trans 2 Predicted_Risk Score:</strong> {edge_data['Predicted_risk_score_t2']}"
                        ...
                        for key, value in edge_data.items():
                            ...
                            if key not in ['from', 'to', 'label', 'color', 'arrows', 'Amount', 'Currency', 'Timestamp',
                                           'is_mule_transaction', 'Predicted_Fraud_Flag', 'Predicted_Risk_Score', 'IP_Address', 'Geo_Location',
                                           ...
                                           'Predicted_fraud_flag_t1', 'Predicted_risk_score_t1', 'Predicted_fraud_flag_t2', 'Predicted_risk_score_t2', 'browser']:
...
def update_account_properties(account_id):
    ...
    Example: {"is_Predicted_fraud": true, "city": "New York"}
    """
    if not driver:
